...
class StockUtils:
    ...
    def getStockAnalyze(self,stockCode,sEndDate="",sStartDate=""):
        ...
        Args:
            ...
            sStartDate (str): 取得交易日期區間的「開始日期」，如傳入空值，預設為「今日-60」，格式: 2024-06-20
        Returns:
            list
                index[0] : 資料日期.
                index[1] : 股票代號.
        ...
        # 如果沒有先用 fetch_from 取到歷史資料，預設抓 30 天的資
        # npPrice  = np.array(tObjStock.price   , dtype='float64') # 股票的歷史價格數據
        # npHigh   = np.array(tObjStock.high    , dtype='float64') # 歷史最高價數據
        # npLow    = np.array(tObjStock.low     , dtype='float64') # 歷史最低價數據
        ...
        # 將 list 轉換為 NumPy 數組
        npHigh   = np.array(listHigh  , dtype='float64') # 歷史最高價數據
        npLow    = np.array(listLow   , dtype='float64') # 歷史最低價數據
        npClose  = np.array(listClose , dtype='float64') # 歷史收盤價
        npVolume = np.array(listVolume, dtype='float64') # 歷史成交量
        # print(F"\t npHigh   = ({type(npHigh)}) {npHigh}")
        # print(F"\t npLow    = ({type(npLow)}) {npLow}")
        # print(F"\t npClose  = ({type(npClose)}) {npClose}")
        # print(F"\t npVolume = ({type(npVolume)}) {npVolume}")
        ...
        # print(F"vma_short = {type(vma_short)} {vma_short}")
        # print(F"vma_mid   = {type(vma_mid)} {vma_mid}")
        # print(F"vma_long  = {type(vma_long)} {vma_long}")

        # 計算VROC (Volume Rate of Change)
        vroc = talib.ROC(npVolume, timeperiod=12)
        # print(F"計算VROC (Volume Rate of Change) = {type(vroc)} {vroc}")
        ...
        # print(F"力量指標 (FI) = {type(force_index)} {force_index}")

        # 標準差通道 (Standard Deviation Channel)
        upper_stddev, lower_stddev = middleband + stddev, middleband - stddev
        # print(F"標準差通道 (Standard Deviation Channel) : upper_stddev = {type(upper_stddev)} {upper_stddev},lower_stddev = {type(lower_stddev)} {lower_stddev}")
        ...
        for i in range(len(listDate)):
            ...
            fPrice  = listPrice[i]  # 股票的歷史價格數據 <class 'float'>
            fHigh   = listHigh[i]   # 歷史最高價數據 <class 'float'>
            fLow    = listLow[i]    # 歷史最低價數據 <class 'float'>
            ...
            dictSignals = {
                ...
                "方差移動平均線 (VARMA)"           : "多頭" if varma[i] > varma[i-1] else "空頭",
            ...
            # print(F"\t dDate = {dDate} ,Price = {fPrice} ,最高價 = {fHigh} ,最低價 = {fLow} ,收盤價 = {nClosePrice} ,成交量 = {fVolume} ,多頭數量 = {iBullishCount}")
            ...
            # 基本訊息也加入
            dictSignals["歷史價格數據"] = fPrice  # 股票的歷史價格數據 <class 'float'>
            dictSignals["歷史最高價"] = fHigh   # 歷史最高價數據 <class 'float'>
            dictSignals["歷史最低價"] = fLow    # 歷史最低價數據 <class 'float'>
            ...
            # listResult.append([dDate.strftime('%Y-%m-%d'),stockName,fClose,iBullishCount,iBearishCount])
    ...
    def saveToExcel(self,listStockInfo, sPath):
        '''
        將數據保存到 Excel 文件中
        ...
        for i in range(len(listStockInfo)):
            ...
            listOutput.append([sDate,stockCode,stockName,fClosePrice,iBullishCount,iBearishCount])
        ...
        # ------------------------------
        # Set 表頭
        columns = ["日期", "股票代號", "股票名稱", "收盤價", "技術指標多頭", "技術指標空頭"]
        ...
        print(f"資料已保存到 = {sFilePath}")

    def saveToGoogle(self,listStockInfo):
        '''
